...
if ( ! function_exists( '_starter_setup' ) ) :
	...
	function _starter_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', '_starter' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'_starter_custom_background_args',
				array(
...
function _starter_scripts() {
	wp_enqueue_style( '_starter-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( '_starter-style', 'rtl', 'replace' );

	wp_enqueue_script( '_starter-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
